...
def load_wins() -> pd.DataFrame:
    ...
    matches23 = (matches23.groupby(['season','team', 'week'], as_index=False).agg(win = ('win', 'sum')))

    matches24 = premier24.copy()
    matches24['team'] = matches24['HomeTeam']
    matches24['win'] = (matches24['FTR'] == 'H').astype(int)
    matches24['season'] = '2024-2025'
    ...
    matches24 = (matches24.groupby(['season','team', 'week'], as_index=False).agg(win=('win', 'sum')))
